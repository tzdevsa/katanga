    ...
    >
      <Link href={`/services/${service.serviceId}`}>
        ...
        >
          <Grid item xs={12}>
            <CardContent>
              <Typography variant="subtitle1" color="whitesmoke" textTransform="uppercase" fontWeight="bold" gutterBottom>
                {service?.name ?? ""}
              </Typography>
              <Typography component="h6" variant="caption" color="white" textTransform="lowercase">
                {service?.description ?? ""}
              </Typography>
            </CardContent>
          </Grid>
